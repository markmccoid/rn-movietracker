...
const ListSearchBar = ({ visible, onCancel = () => null }) => {
  // Uinsg state for storing input in components
  // NOTE: when input changed we set both this local state AND
  //       the state in Overmind (oSaved.searchFilter)
  const [localInput, setLocalInput] = React.useState("");

  const state = useOState();
  const actions = useOActions();
  const {
    filterData: { ignoreFilterOnSearch },
  } = state.oSaved;
